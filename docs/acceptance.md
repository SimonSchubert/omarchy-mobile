...
| W7 | pass | **Added here.** `mobile.lua` unbinds upstream's two pointer drags, and after a real `hyprctl reload` no binding is keyed on a mouse button at all. Checked with the drag too: the SUPER drag that had walked a floating window to `[-21, -82]` and the SUPER right-drag that had cut its height to 600 both leave it exactly where they find it |
...
| AC | Status | Note |
| --- | --- | --- |
| A1–I4 | todo | No `scripts/style-check.sh` yet. The surfaces here follow moarchy's radii, roles, weights and 44px targets by hand, unchecked. C3 and I1a are the exceptions below |
| C3 | holds | Every `subdued` now comes from `Theme.subduedOn()` or `subduedOnContainer()`, and no surface fixes an alpha. Checked by arithmetic on 2026-09-12 against the `colors.toml` of all 22 themes as shipped, each against the fill its text is drawn on: the four that had picked their own number -- the drawer and the bar at 0.55, the carousel at 0.6, Wi-Fi at the 0.62 this AC names -- were below 4.5:1 on 18, 15, 11 and 11 of the 22, worst 2.36:1 on rose-pine. All four clear it on all 22 now, lowest 4.51:1. The one place that cannot is the shade's lit tile, where the ink is the background and the fill is the accent: several themes do not have 4.5:1 between those two at any alpha, and `readableOn` returns the full ink, topping out at 3.14:1. Not a screenshot check -- no `vm-selftest.sh` line reads a colour |
...
| D5, D6, D7 | pass | |
| D8 | changed | AI agent is visible on an image built from 2026-09-12 on, and hidden on every one built before it. `omarchy-default-agent` installs through mise; mise is a pin now (`[pkg.mise-bin]`, moved out of `vm/packages/omitted`) rather than an omission, so the row's guard is unchanged and says yes once the apps tier carries it. **Seen on a guest, not on a built image** -- with the pinned `mise-bin` installed by hand on 2026-09-12, `settings rows` on `apps.default` answers `agent nav AI agent 1`, which is the first time the row has been visible at all; no image has been rebuilt around the pin. The rows are upstream's thirteen, writing `omarchy-mobile-agent open <name>` rather than upstream's own action, which is P: the wrapper writes the drawer tile and then execs `omarchy-default-agent` |
...
| O15 | pass | |
| P1 | pass | One `omarchy-mobile-agent.desktop`, rewritten. This port never shipped the per-agent tiles moarchy's P1 was written against, so there was nothing to clean up -- which is also why P8 below is a different criterion here rather than the same one |
| P2 | pass | `Name`, `Exec` and `Icon` move together, with `X-Omarchy-Mobile-Agent` rather than moarchy's `X-Moarchy-Agent` |
| P3 | pass | Absent and junk `defaults/agent` both answer the setup tile. `current_agent()` is moarchy's, unchanged but for the thirteen names it validates against |
| P4 | pass | **Changed.** **The one place this port reverses moarchy.** moarchy writes `Icon=/usr/share/moarchy/agents/<name>.svg` because no agent has an icon in Adwaita, breeze or hicolor, so an absolute path was the only escape from `system-run`. That reason stops applying once the icons are ours: this ships fourteen as `omarchy-mobile-agent-<name>` and `omarchy-mobile-agent` under `~/.local/share/icons/hicolor/scalable/apps/`, so the lookup is ordinary. It is the argument the three shell-screen entries beside it already make (`omarchy-mobile-wifi.desktop`): a name of our own cannot be shadowed by a theme's monochrome panel glyph, and the tile's `Icon=` stays portable instead of naming a path |
| P5 | pass | Three lists checked against upstream's `omarchy:args=`, not against each other: `omarchy-mobile-agent list`, the choice-row ids on `apps.default.agent`, and the icon basenames with the prefix stripped. **Thirteen, not moarchy's nine** -- upstream grew Cursor, Hermes, Muse and OpenClaw after moarchy's set was made |
| P6 | pass | **Changed.** Both halves, now that a guest carries mise. Without it `write_shim` writes nothing at all -- the wrapper's whole body is a `mise` call, so seeding one would put eleven names on PATH that answer `mise: command not found` and make `omarchy-cmd-present` lie to the one launcher that asks it, and the row is hidden on that same condition (F8) -- and the tile is still written. With it, eleven of thirteen (P8), and `grep -l mise ~/.local/bin/*` finds those eleven and nothing else. Seen on a guest with the pinned `mise-bin` installed by hand on 2026-09-12, not on an image built around it |
| P7 | pass | A decoy at `~/.local/bin/claude` that says nothing about mise survives `seed` byte-identical -- checked with mise present, which is the only state in which `write_shim` gets as far as reading the file. Upstream depends on this too: `user_install()` in `omarchy-default-agent` reads that same file and treats a non-mise wrapper as the user's own install |
| P8 | pass | Neither `~/.local/bin/hermes` nor `~/.local/bin/openclaw` exists after a seed on a guest that has mise, and while mise is absent nothing is written for any agent at all, those two included. **Changed.** **Not moarchy's P8**, which was about deleting per-agent tiles a flashed phone still carried; this port never wrote any. The criterion that earns the slot here is Hermes and OpenClaw, which are never shimmed. Each has an installer of its own (`omarchy-install-hermes-cli`, `omarchy-install-openclaw-cli`) that owns `~/.local/bin/<name>`, writes a marker line into it and refuses to touch anything foreign; Hermes needs its interpreter pinned, which a bare `mise use` cannot say, and OpenClaw comes from a pacman package. A mise wrapper at either path would leave the real installer standing aside from a stub that installs the wrong thing. Both still install from the row and the tile, because `omarchy-default-agent` calls those installers directly |
| P9 | pass | `omarchy-default-agent codex` behind the script's back, then `omarchy-mobile-agent entry` with no argument repairs the tile |
| P10 | pass | `omarchy-shell settings openAt apps.default.agent` answers `ok` and leaves `settings page` there with thirteen choice rows drawn |
| P11 | pass | All thirteen names in `Keywords`. This is the criterion the whole section exists for: searching the drawer for "agent" answered nothing, and it answered nothing twice over -- no tile, and the settings row that would have matched withheld by its mise guard, which the drawer honours (O7) |
...
**mise installed an agent that could not run.** `mise use -g
npm:@xai-official/grok` needs no node to *install* -- mise fetches the tarballs
itself, and did it in seven seconds on a guest with neither node nor npm -- but
what it unpacks is a shim whose last line is `exec node`, so the agent answered
`exec: node: not found`. Upstream never meets this: `install/user/mise-work.sh`
runs `mise use -g node@latest` while `omarchy-provision-user` sets a user up,
and this image seeds `/etc/skel` and runs no provisioning of its own. `nodejs`
and `npm` are in `vm/packages/apps` now, from ALARM's extra at 26.8.2-1 -- the
same version mise resolves `node@latest` to -- and with them `~/.local/bin/grok`
answers `grok 1.0.30` and the tile opens Grok's device sign-in in the browser.
